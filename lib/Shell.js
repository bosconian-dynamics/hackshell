...
import ChatsController from './commands/Chats'
import ScriptsController from './commands/Scripts'
...
class Shell {
  constructor( config = {} ) {
    ...
    this.commands = new Map(
      [
        new ChatsController( this.writeOut.bind(this), config.chatDelay ),
        new ScriptsController( this )
      ].map( commandDomain => [ commandDomain.name, commandDomain ] )
    )

    // Add a CommandDomain for user scripts
    this.commands.set( Shell.USER_DOMAIN, new CommandDomain( Shell.USER_DOMAIN ) )
